...
template <typename Dtype>
void DeconvlutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    for (int i = 0; i < bottom.size(); ++i) {
        ...
        for (int n = 0; n < this->num_; ++n) {
            this->backward_cpu_gemm(bottom_data + n * this->bottom_dim_, weight,
                top_data + n * this->top_dim);
            if (this->bias_term_) {
                const Dtype* bias = this->blobs_[i]->cpu_data();
...
template <typename Dtype>
void DeconvlutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
