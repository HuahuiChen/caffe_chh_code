...
__global__ void copy_kernel(const int n, const int height, cosnt int width,
...
void CropLayer<Dtype>::crop_copy_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top, const vector<int>& offsets,
    ...
    if (cur_dim + 2 < top[0]->num_axes()) {
        ...
        const int src_outer_stride = 
            bottom[0]->shape(cur_dim) * bottom[0]->shpae(cur_dim + 1);
...
void CropLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    if (propagate_down[0]) {
        ...
        std::vector<int> indices(top[0]->num_axes(), 0);
        crop_copy_gpu(bottom, top, offsets, indices, 0, top_diff, bottom_diff, false);
    }
}
