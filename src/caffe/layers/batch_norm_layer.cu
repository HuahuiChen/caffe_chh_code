...
void BatchNormLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    if (use_global_stats_) {
        ...
        caffe_gpu_scale(variance_.count(), scale_factor,
            this->blobs_[0]->gpu_data(), meam_.mutable_gpu_data());
        caffe_gpu_scale(variance_.count(), scale_factor,
            this->blobs_[1]->gpu_data(), variance_.mutable_gpu_data());    
    } else {
        caffe_gpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim,
            1. / (num * spatial_dim), bottom_data, spatial_sum_multiplier_.gpu_data(),
...
void BatchNormLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, 
    ...
    caffe_gpu_gemv<Dtype>(CblasNoTrans, num, channels_, 1.,
        ...
        mean_.mutable_gpu_data());
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, channels_, 1, 
        1, batch_sum_multiplier_.gpu_data(), mean_.gpu_data(), 0.,
        num_by_chans_.mutable_gpu_data());
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels_ * num,
        spatial_dim, 1, 1., num_by_chans_.gpu_data(),
