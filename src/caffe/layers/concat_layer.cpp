...
void ConCatLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    const ConcatLParameter param = this->layer_param_.concat_param();
...
void ConCatLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    const ConcatLParameter& concat_param = this->layer_param_.concat_param();
    ...
    for (int i = 1; i < bottom.size(); ++i) {
        CHECK_EQ(num_axes, bottom[i]->num_axes())
            << "All inputs must have the sam axes.";
        for (int j = 0; j < num_axes; ++j) {
            if (j == concat_axis_) {
                continue;
            }
            CHECK_EQ(top_shape[j], bottom[j]->shape(j))
